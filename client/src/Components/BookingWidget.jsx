...
export default function BookingWidget({ place }) {
  ...
  useEffect(() => {
    ...
    const fetchBookingDetails = async () => {
      const { data } = await axios.get(`http://localhost:4000/booking/${place._id}`);
  ...
  async function bookThisPlace() {
    ...
    try {
      const { data } = await axios.post("http://localhost:4000/booking", {
      ...
      await axios.put("http://localhost:4000/user/bookings", {
        bookingId: data._id,
        bookedPlace: place._id,
      });
      await axios.put(`http://localhost:4000/place/${place._id}`, { checkIn : formattedDate(checkIn), checkOut : formattedDate(checkOut)});
